...
def vlm_collate_fn(batch):
    ...
    if hasattr(pixel_data[0], 'keys'):
        ...
        max_imgs = max(d[list(d.keys())[0]].shape[0] for d in pixel_data)
        pixel_values = {}
        for k in pixel_data[0].keys():
            tensors = []
            for d in pixel_data:
                t = d[k]
                if t.shape[0] < max_imgs:
                    # Pad with zeros
                    pad_shape = (max_imgs - t.shape[0],) + t.shape[1:]
                    padding = torch.zeros(pad_shape, dtype=t.dtype, device=t.device)
                    t = torch.cat([t, padding], dim=0)
                tensors.append(t)
            pixel_values[k] = torch.stack(tensors)
    else:
        max_imgs = max(d.shape[0] for d in pixel_data)
        tensors = []
        for d in pixel_data:
            if d.shape[0] < max_imgs:
                pad_shape = (max_imgs - d.shape[0],) + d.shape[1:]
                padding = torch.zeros(pad_shape, dtype=d.dtype, device=d.device)
